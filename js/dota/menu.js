...
DOTA.MENU.MenuItem.prototype = {
	...
	setFlag : function(flag){
		this.isChecked = flag;
		this.check.innerHTML = this.isChecked ? "√" : "&nbsp;";
	},
	getFlag : function(){
...
DOTA.MENU.SubMenu.prototype = {
	...
	getParent : function(obj){
		while(obj && obj.className != "d_menu"){
			obj = obj.parentNode;
		}
		return obj;
	},
	show : function(parent, level){
		...
		if( !this.isSet ){
			...
			if(level === 0){
				...
				pos = DOTA.dom(parent).offset(this.getParent(parent)), wid = parent.offsetWidth;
...
DOTA.MENU.Menu.prototype = {
	...
	setFlag : function(mainIndex, subIndex, flag){
		this.mainMenu.menus[mainIndex].subMenu.menus[subIndex].setFlag(flag);
	},
	getFlag : function(mainIndex, subIndex){
		return this.mainMenu.menus[mainIndex].subMenu.menus[subIndex].getFlag();
	},
	setText : function(mainIndex, subIndex, text){
		this.mainMenu.menus[mainIndex].subMenu.menus[subIndex].setText(text);
	},
	getText : function(mainIndex, subIndex){
		return this.mainMenu.menus[mainIndex].subMenu.menus[subIndex].getText();
	},
	dispose : function(){
