...
DOTA.Animation.prototype = {
	...
	animate : function(show, speed, callback){
		...
		var cfg = this.config, el = this.element, style = DOTA.dom(el).currentStyle(), bg = this.moveBG,
			width = parseInt(style.width), height = parseInt(style.height), offset = DOTA.dom(el).offset(),
		...
		var timerID = setInterval(function(){
			...
			DOTA.dom(bg).setOpacity(w / width * 100);
			if(++frame > maxFrame){
